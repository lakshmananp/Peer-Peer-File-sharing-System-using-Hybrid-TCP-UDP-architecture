...
printf("\n\n\n****************************PHASE 1 BEGINS ******************************");
printf("\n\nPhase 1 : File Server1 has UDP port number =%d and IP address =%s",ntohs(client.sin_port),inet_ntoa(client.sin_addr));

strcpy(buffer,"File_Server1 51991");
...
printf("\n\n\n****************************PHASE 3 BEGINS ******************************");
 ...
 printf("\n\n\nPhase3 : File Server1 has TCP port number :%d  And IP address :%s",ntohs(servaddr.sin_port),inet_ntoa(servaddr.sin_addr));
 printf("  ");

 
 // The file server keeps on litening for clients for connection in a infinite while loop 
 while(1)
 { 
 listen(sockfd,2);
 len=sizeof(cliaddr);
 ...
 {
 printf("\n\nPhase3 : File Server1 has received a request from Client1 for Doc1");
 strcpy(buff,"");
 strcpy(buff,"doc1");
 }
 else if(strstr(buff,"doc2") != NULL)
 {
 printf("\n\nPhase3 : File Server1 has received a request from Client2 for Doc2");
 strcpy(buff,"");
 strcpy(buff,"doc2");
 }
...
// puts(buff);
 ...
 printf("\n\nPhase3 : File Server1 has sent %s to Client1",buff);
 else if(strcmp(buff,"doc2")==0)
 printf("\n\nPhase3 : File Server1 has sent %s to Client2",buff);
 puts("");
//close(sockfd);
}
close(sockfd);
